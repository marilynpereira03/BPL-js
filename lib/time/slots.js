...
const getEpochTime = (time) => {
	...
	let d = this.beginEpochTime() || beginEpochTime();
	let t = d.getTime();
...
const getTime = (time) => {
	return this.getEpochTime(time) || getEpochTime(time);
};

const getRealTime = (epochTime) => {
	if (epochTime === undefined) {
		epochTime = this.getTime() || getTime();
	}
	let d = this.beginEpochTime() || beginEpochTime();
...
const getNextSlot = () => {
	let slot = this.getSlotNumber() || getSlotNumber();

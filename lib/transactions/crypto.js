...
const getBytes = (transaction, skipSignature, skipSecondSignature) => {
	...
	case 1: // Signature
		assetBytes = this.getSignatureBytes(transaction.asset.signature) || getSignatureBytes(transaction.asset.signature);
...
const getId = (transaction) => {
	let bytes = this.getBytes(transaction) || getBytes(transaction);
...
const getHash = (transaction, skipSignature, skipSecondSignature) => {
	let bytes = this.getBytes(transaction, skipSignature, skipSecondSignature) || getBytes(transaction, skipSignature, skipSecondSignature);
...
const sign = (transaction, keys) => {
	let hash = this.getHash(transaction, true, true) || getHash(transaction, true, true);
...
const secondSign = (transaction, keys) => {
	let hash = this.getHash(transaction, false, true) || getHash(transaction, false, true);
...
const verify = (transaction, network) => {
	...
	let hash = this.getHash(transaction, true, true) || getHash(transaction, false, true);

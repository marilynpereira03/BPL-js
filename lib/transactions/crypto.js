...
function getBytes(transaction, skipSignature, skipSecondSignature) {
	...
	case 6: // Smart Contract
		if (transaction.asset.smartContract.type !== null) {
			assetBytes = new Buffer(transaction.asset.smartContract.type, 'utf8');
			assetSize = assetBytes.length;
		}
		break;
	case 7: // Sidechain
		// if (transaction.asset.hash !== null) {
		// 	assetBytes = new Buffer(transaction.asset.hash, 'utf8');
		// 	assetSize = assetBytes.length;
		// }
		break;
	}
